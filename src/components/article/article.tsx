...
import { ArticleControls, Articles, ChevronWrapper, OuterLayout } from "./article.styles"
...
export default class Article extends Component<any, {newArticles: ArticleConfig[], current: number}> {
  ...
  getNewArticles = (start: number) => {
    let offsetStart = start - 2;

    return ArticleConfigs.slice(offsetStart < 0 ? 0 : offsetStart, offsetStart + 5)
  }

  changeSelected = (amount: number) => {
    let newCurrent = this.state.current + amount
    newCurrent = newCurrent < 0 ? 0 : newCurrent
    newCurrent = newCurrent < ArticleConfigs.length - 1 ? newCurrent : ArticleConfigs.length - 1;
    this.setState({
  ...
  render(): React.ReactNode {
    return (
      <OuterLayout>
        <InnerLayout>
          <FirstWord>Latest</FirstWord><SecondWord>Articles</SecondWord>
          <BorderUnderline/>
          <ArticleControls>
            ...
            <ChevronWrapper onClick={() => this.changeSelected(1)}>
              ...
              {
                this.state.current < ArticleConfigs.length - 1 ? <ChevronRight/> : null
              }
            </ChevronWrapper>
          </ArticleControls>
        </InnerLayout>
